...
setwd("../../../DeepDiveR/deepdive_project_main_branch_runs/dicynodon/dicynodon_models/simulations_20250312_lstm64_32_d64_32/")
...
# Temnospondyli and Synapsida (and DICYNODON) year vector
...
plot_data <- data.frame(
  ...
  step_line_chart <- ggplot(plot_data, aes(x = year, y = columns_list, color = columns_labels)) +
    ...
    scale_color_manual(values = c("Region 1" = "red",
                                  ...
                                  "Region 4" = "orange1")) +
    labs(x = "Time (Ma)",
         y = "Number of Dicynodon Occurrences",
         color = "Region") +
    coord_geo(xlim = c(-320, -190),
              expand = FALSE,
              clip = "on",
 ...
 ######################## 3. N_LOCS GRAPH ####################################
  ...
  plot_data <- data.frame(
    ...
    step_line_chart <- ggplot(plot_data, aes(x = year, y = columns_list, color = columns_labels)) +
      ...
      scale_color_manual(values = c("Region 1" = "red",
                                    ...
                                    "Region 4" = "orange1")) +
      labs(x = "Time (Ma)",
           y = "Number of Dicynodon Localities",
           color = "Region") +
      coord_geo(xlim = c(-320, -190),
                expand = FALSE,
                clip = "on",
  ...
  #################### 4. ORIGINATION + EXTINCTION EVENTS#######################
    ...
    step_line_chart <- ggplot(plot_data, aes(x = year, y = columns_list, color = columns_labels)) +
      ...
      scale_color_manual(values = c("Extinction" = "red",
                                    "Origination" = "blue")) +
      labs(x = "Time (Ma)",
           y = "Dicynodon Extinction and Origination Events",
           color = "Region") +
      coord_geo(xlim = c(-320, -190),
                expand = FALSE,
                clip = "on",
  ...
  # N_Endemics
  plot_graph(year, data$n_endemics, "Number of Dicynodon Endemics", "feature_plots_formatted/n_endemics_formatted.pdf")

  # N_Singletons
  plot_graph(year, data$n_singletons, "Number of Dicynodon Singletons", "feature_plots_formatted/n_singletons_formatted.pdf")

  # Range_through_div
  plot_graph(year, data$range_through_div, "Dicynodon Range Through Diversity", "feature_plots_formatted/range_through_div_formatted.pdf")

  # Net diversity
  net_diversity = data$origination_events - data$extinction_events
  plot_graph(year, net_diversity, "Net Dicynodon Diversity", "feature_plots_formatted/net_diversity_formatted.pdf")
  ...
  plot_data <- data.frame(
    ...
    step_line_chart <- ggplot(plot_data) +
      ...
      scale_x_reverse() +
      labs(x = "Time (Ma)", y = "Dicynodon Diversity Predictions") +
      coord_geo(xlim = c(-320, -190), expand = FALSE, clip = "on",
                dat = list("international epochs", "international periods"),
                abbrv = list(TRUE, FALSE), pos = list("bottom", "bottom"),
                alpha = 1, height = unit(1.5, "line"), rot = 0,
