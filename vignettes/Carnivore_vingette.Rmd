...
It produces formatted input data and a configuration file that can then be exectued in the command line, enabling a clearer and more readily reproducable workflow.
...
With the argument 'r=10' we are randomly resampling occurrence ages 10 times from their stratigraphic ranges.
For your final analysis, you may want to make more replicates e.g. 100.
...
## 1. Move out of current wd with this file, clone down the "deepdive" repository from GitHub

<https://github.com/DeepDive-project/deepdive> \^ This will containt the programs for running the program

## 2. Run Deep Dive in terminal (command line)

Go back to the wd with this file!
We will now run the programs in that repo\^ on your files
...
### 2.A. If you want to specify a wd for all the output files to go to and if yuo want to specify the number of CPUs used for the parallelized simulations, you can do so with the following flags:
